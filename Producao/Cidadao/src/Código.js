...
function findDuplicateCDAs(sheet, cdasToCheck) {
  ...
  data.forEach(row => {
    // Garante que row[0] seja tratado como string e não seja null/undefined
    const cdaValue = (row[0] || '').toString();
    const cdasInSheet = cdaValue.split(',').map(cda => cda.trim());
    ...
    if (status && status.toLowerCase() !== 'indeferido') {
